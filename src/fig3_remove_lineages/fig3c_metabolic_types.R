...
#alldata <- readRDS('data/temp/fig3_test_grid_data.RDS')
#alldata <- readRDS('data/temp/CF_freenon_evo704_nomut_k701_t200010fig3grid.RDS')
...
shadefig <- for_plot2 %>% 
  ggplot() +
  geom_area(aes(x = rel_time, y = count, fill = as.factor(shadetype_n)),
  ) + 
